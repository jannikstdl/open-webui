<script lang="ts">
	...
	let enableCallFeature = false; // FI-TS_custom 05.02.2025 - Disabled until we run a vision model
			...
			>
				...
				<div class="w-full relative">
					{#if atSelectedModel !== undefined || selectedToolIds.length > 0 || webSearchEnabled || ($settings?.webSearch ?? false) === 'always' || imageGenerationEnabled || codeInterpreterEnabled}
						<div
							class="px-7 pb-0.5 pt-1.5 text-left w-full flex flex-col absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white dark:from-gray-900 z-10"
						>
							{#if selectedToolIds.length > 0}
								<div class="flex items-center justify-between w-full">
									<div class="flex items-center gap-2.5 text-sm text-gray-600 dark:text-gray-500">
										<div class="pl-1">
			...
			>
				<div class="">
					...
					{:else}
						...
						>
							<div
								class="relative w-full rounded-3xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-2 pt-1 pb-0.5 mb-3 mx-5 shadow-[0_9px_9px_0_rgba(0,0,0,0.01),0_2px_5px_0_rgba(0,0,0,0.06)] transition-colors duration-150 ease-in-out"
								dir={$settings?.chatDirection ?? 'LTR'}
							>
								<!-- FI-TS_custom 05.02.2025 - Custom Input styling -->
								{#if files.length > 0}
								...
								<div class=" flex justify-between mt-1.5 mb-2.5 mx-0.5 max-w-full">
									<div class="ml-1 self-end gap-0.5 flex items-center flex-1 max-w-[80%]">
										...
										<div class="flex gap-0.5 items-center overflow-x-auto scrollbar-none flex-1">
											{#if $_user}
												...
												{#if /* FI-TS_custom - also deactivate for admin users $_user.role === 'admin' ||*/ $_user?.permissions?.features?.code_interpreter}
									...
									<div class="self-end flex space-x-1 mr-1 shrink-0">
										{#if !history?.currentId || history.messages[history.currentId]?.done == true}
											<Tooltip content={`${$i18n.t('Record voice')} (Nicht im AdZ)`}>
												<button
										...
										{#if !history.currentId || history.messages[history.currentId]?.done == true}
											{#if prompt === '' && enableCallFeature}
												<!-- FI-TS_custom 05.02.2025 - enableCallFeature -->
